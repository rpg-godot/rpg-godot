...
uniform float zoom2 = 10;
uniform float seed = 0.0;
uniform float seed2 = 12.9898;
uniform float seed3 = 78.233;
uniform vec2 movement = vec2(0, 0);

float rand(vec2 coord){
	// prevents randomness decreasing from coordinates too large
	coord = mod(coord, 10000.0);
	// returns "random" float between 0 and 1
	return fract(sin(dot(coord, vec2(seed2,seed3))) * (43758.5453 + seed));
}
